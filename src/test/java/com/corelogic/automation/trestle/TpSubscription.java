...
public class TpSubscription extends Wraper {
	
	private static volatile TpSubscription subscriptions = null;
	public String stringData = "";
	public Map<String, String> data = null;

	public TpSubscription setStringData(String stringData) {
		this.stringData = stringData;
		return TpSubscription.getInstance();
	}
	
	public TpSubscription setData(Map<String, String> data) {
		this.data = data;
		return TpSubscription.getInstance();
	}

	public static TpSubscription getInstance() {
		if (subscriptions == null) {
			synchronized (TpSubscription.class) {
				if (subscriptions == null) {
					subscriptions = new TpSubscription();
				}
			}
		}
	...
	public void subscriptionTP() {
		String methodName="subscriptionTP";
		subscriptionOption();
		if(isObjectExist("//*[@id='btnTpSelect']", Using.XPATH, TIMEOUT)) {
			passLogInfo(methodName, "TP subscription option is available");	
			click("//*[@id='btnTpSelect']", Using.XPATH, TIMEOUT);
			if(isObjectExist("//*[contains(text(),'Primary Contact Information')]", Using.XPATH, TIMEOUT)) {
				...
				if(isObjectExist("//*[contains(text(),'Company Information')]", Using.XPATH, TIMEOUT)) {
					...
					findElement(By.xpath("//*[@id='BusinessEntityName']")).sendKeys(data.get("TPname"));
