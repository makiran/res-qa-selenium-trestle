...
public class MloCompanyPage extends Wraper {
	
	private static volatile MloCompanyPage mlocompanypage = null;
	public String stringData = "";
	public Map<String, String> data = null;

	public MloCompanyPage setStringData(String stringData) {
		this.stringData = stringData;
		return MloCompanyPage.getInstance();
	}
	
	public MloCompanyPage setData(Map<String, String> data) {
		this.data = data;
		return MloCompanyPage.getInstance();
	}

	public static MloCompanyPage getInstance() {
		if (mlocompanypage == null) {
			synchronized (MloCompanyPage.class) {
				if (mlocompanypage == null) {
					mlocompanypage = new MloCompanyPage();
				}
			}
		}
		return mlocompanypage;
	}
	public void verifyMloCompanyPage(){
		String methodName="verifyMloCompanyPage";
		click("//*[@id='navbar-collapse']/ul[1]/li[3]/a", Using.XPATH, TIMEOUT);
		click("//*[@href='/MLO/Account/EditComp']", Using.XPATH, TIMEOUT);
		if(isObjectExist("//*[contains(text(),'Company Information')]", Using.XPATH, TIMEOUT)) {
			...
			if(isObjectExist("//*[@class='active' and @id='tabCompany']", Using.XPATH, TIMEOUT)) {
				...
				if(findElement(By.xpath("//*[@id='BusinessName']")).getAttribute("value").equals(data.get("MLOname")))
	...
	public void verifyMloCompanyDesc(){
		String methodName="verifyMloCompanyDesc";
